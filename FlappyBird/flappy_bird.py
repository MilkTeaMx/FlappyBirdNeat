...
def main(genomes, config):
    #this function needs to return fitness of all birds
    
    nets = []
    ge = []
    birds = []
    
    for _, g in genomes: #genomes is the nueral network object, (id, object)
    ...
    while run:
        ...
        for x, bird in enumerate(birds):
            ...
            state = (bird.y, abs(bird.y - pipes[pipe_ind].height), abs(bird.y - pipes[pipe_ind].bottom))
            output = nets[x].activate(state) #output is list
...
def run(config_path):
    config = neat.config.Config(neat.DefaultGenome, neat.DefaultReproduction, neat.DefaultSpeciesSet, 
                                neat.DefaultStagnation, config_path) #variables are from config.txt
    
    p = neat.Population(config)
    p.add_reporter(neat.StdOutReporter(True)) #gives statistics of algo
    stats = neat.StatisticsReporter()
    p.add_reporter(stats)
    
    winner = p.run(main(), 50) #run fitness function for 50 generations. 
